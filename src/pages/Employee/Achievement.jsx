...
import {
  Button, Col, Form, Input, Modal, Row, Select, Upload
} from "antd";
  ...
  const formValidate = () => {
    onedit
      ? form
        .validateFields()
        .then((values) => {
          selectedRowKeys[1][0]?.id && editAchievement({ ...values, id: selectedRowKeys[1][0]?.id });
          setOnedit(false);
        })
        .catch((info) => {
      ...
      : form
        ...
        .then((values) => {
          ...
          fmData.append("photoCertificate", file);
...
export default connect((achievementReducer, employeeReducer, usersDataReducer), {
  deleteAchievement, editAchievement, saveAchievement, getAchievementUserId, getEmployeeBranch
})(Achievement);
