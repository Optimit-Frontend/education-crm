...
import teachingHourReducer, {
  ...
  // eslint-disable-next-line import/named
  editTeachingHours,
  getTeachingHour, saveTeachingHours,
} from "../../reducer/teachingHourReducer.js";
...
function Attendance({
  usersDataReducer, getWorkExperienceByUserId, workExpirenceReducer,
  employeeReducer, getUserLists, businessBranchesReducer, subjectReducer, getSubject, getClassesAll, classReducer,
  getBusinessBranch, teachingHourReducer, getTeachingHour, editTeachingHours, saveTeachingHours, deleteTeachingHours
}) {
    ...
    onedit
      ...
      : form
        ...
        .then((values) => {
          saveTeachingHours({
            ...values,
            lessonHours: parseInt(values.lessonHours, 10)
          });
          setOnedit(false);
        })
        .catch((info) => {
        ...
        {selectedRowKeys[0].length === 1 && (
          <button
            onClick={() => {
              ...
              form.setFieldValue("subjectId", selectedRowKeys[1][0]?.subjectId);
...
export default connect(
  (
    ...
    workExpirenceReducer, teachingHourReducer
  ), {
    getEmployeeBranch,
    getUserLists,
    getBusinessBranch,
