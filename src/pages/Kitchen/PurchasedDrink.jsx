...
import purchasedDrinkReducer, {
  deletePurchasedDrink,
  editPurchasedDrink,
  getPurchasedDrinkBranch,
  getPurchasedDrinkWearehouse,
  savePurchasedDrink
} from "../../reducer/purchasedDrinkReducer";
...
function PurchasedDrink({
  ...
  useEffect(() => {
    ...
    form.resetFields();
    setSelectedRowKeys([[], []]);
  }, [purchasedDrinkReducer?.changeData]);
  ...
  useEffect(() => {
    ...
    if (pageSize >= 100) {
      ...
      navigate(`/kitchen/purchasedDrink?page=${pageCount}&size=100`);
    } else if (pageSize >= 50) {
      setPageData((prev) => {
        return { ...prev, size: 50 };
      });
      navigate(`/kitchen/purchasedDrink?page=${pageCount}&size=50`);
    } else if (pageSize >= 20) {
      setPageData((prev) => {
        return { ...prev, size: 20 };
      });
      navigate(`/kitchen/purchasedDrink?page=${pageCount}&size=20`);
    } else {
      setPageData((prev) => {
        return { ...prev, size: 10 };
      });
      navigate(`/kitchen/purchasedDrink?page=${pageCount}&size=10`);
    }
  }, []);

  const handleDelete = (arr) => {
    arr?.map((item) => {
      deletePurchasedDrink(item);
      return null;
    });
  };
  ...
  const onChange = (pageNumber, page) => {
    ...
    navigate(`/kitchen/purchasedDrink?page=${pageNumber}&size=${page}`);
  };
  ...
  const formValidate = () => {
    onedit
      ? form
        .validateFields()
        .then((values) => {
          selectedRowKeys[1][0]?.id && editPurchasedDrink({
            ...values,
      ...
      : form
        .validateFields()
        .then((values) => {
          savePurchasedDrink({ ...values, branchId: usersDataReducer?.branch?.id });
          setOnedit(false);
        })
        .catch((info) => {
          console.error("Validate Failed:", info);
  ...
  return (
    <div>
      <h3 className="mb-5 text-2xl font-bold">Sotib Olingan Ichimliklar</h3>
      <div className="mb-3 flex items-center justify-between gap-5">
        <div>
          <Select
            onChange={(e) => {
              e ? getPurchasedDrinkWearehouse({
                page: pageData.page,
                size: pageData.size,
                warehouseId: e,
              }) : getPurchasedDrinkBranch({
          ...
          <Row gutter={12}>
            <Col span={24}>
              ...
              <Form.Item
                key="literQuantity"
                name="literQuantity"
                label={<span className="text-base font-medium">Litr miqdori</span>}
                rules={[
                  {
                    required: true,
                    message: "Litr miqdorini kiriting",
                  },
                ]}
              >
                <InputNumber placeholder="Litr miqdorini kiriting..." className="w-full" />
              </Form.Item>
              <Form.Item
                key="unitPrice"
                name="unitPrice"
                label={<span className="text-base font-medium">Birlik narxi</span>}
                rules={[
                  {
                    required: true,
                    message: "Ichimlik birlik narxini kiriting",
                  },
                ]}
              >
                <InputNumber placeholder="Ichimlik birlik narxini kiriting..." className="w-full" />
              </Form.Item>
              <Form.Item
                key="totalPrice"
                name="totalPrice"
                label={<span className="text-base font-medium">Umumiy narxi</span>}
                rules={[
                  {
                    required: true,
                    message: "Ichimlik umumiy narxini kiriting",
                  },
                ]}
              >
                <InputNumber placeholder="Ichimlik umumiy narxini kiriting..." className="w-full" />
              </Form.Item>
              <Form.Item
                key="description"
                name="description"
      ...
      <CustomTable
        ...
        totalItems={purchasedDrinkReducer?.purchasedDrinkTotalCount}
        tableData={purchasedDrinkReducer?.purchasedDrink?.map((item) => {
          return {
            ...item,
